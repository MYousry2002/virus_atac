...
for cell in cell_types:
    ...
    sns.scatterplot(data=df, x='ATAC_signal', y=cell, s=20, alpha=0.7)
    plt.title(f"ATAC Signal vs MPRA Activity – {cell}")
    plt.xlabel("ATAC Signal (mean per tile)")
    plt.ylabel("MPRA Activity")
    plt.tight_layout()
    plt.savefig(f"../results/atac_vs_mpra_{cell}_scatter.png", dpi=300)
    plt.close()
...
for cell in cell_types:
    ...
    # Inactive points
    sns.scatterplot(data=df[~df[active_col]], x='ATAC_signal', y=cell, s=20, alpha=0.5, label='Inactive', color='steelblue')
    
    # Active points
    sns.scatterplot(data=df[df[active_col]], x='ATAC_signal', y=cell, s=20, alpha=0.5, label='Active', color='orange')
    
    plt.title(f"ATAC Signal vs MPRA Activity – {cell}")
    plt.xlabel("ATAC Signal (mean per tile)")
    plt.ylabel("MPRA Activity")
    plt.legend(title="MPRA Status")
    plt.tight_layout()
    plt.savefig(f"../results/atac_vs_mpra_{cell}_scatter.png", dpi=300)
    plt.close()